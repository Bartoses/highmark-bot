...
- **Portal** Settings → Operator Intelligence is now a per-person config (Person/Role/Locations/Detail level/Focus areas/Delivery/Channels). Email/Push channels are stubbed "coming soon". Deferred (Phase 2+): drag-drop dashboard widgets, midday briefings, email/push, AI-module plug-in framework (priorityEngine `category→score` + the card registry are the extension points). Tests: `testOperatorIntelligence2` (36 assertions).
