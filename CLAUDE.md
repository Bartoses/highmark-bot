...
- **Outcomes vocabulary:** spam · lead · customer · booking · support · voicemail · transferred · completed · no_answer. (Transcript/summary/spam_score/lead_score columns exist now but are populated by later phases: Phase 2 realtime AI receptionist, Phase 3 lead qualification, Phase 4 shared spam network.)
...
22. **Voice AI Phase 1 — go live** — apply `db1_voice.sql` in DB1, buy/point a Twilio voice number's Voice webhook at `POST /voice/incoming`, add a `voice_numbers` row (+ optional `voice_agents` row with `forwarding_number`/`business_hours`) for CSR/REA, then place a live call and confirm the greeting → forward/voicemail and a `voice_calls` row. Then build Voice AI Phase 2 (realtime AI receptionist via Twilio Media Streams), Phase 3 (lead qualification + missed-call SMS recovery), Phase 4 (shared spam network), Phase 5 (industry agents). Portal "Voice AI" page consumes `GET /portal/api/voice/calls`.
