...
### Messaging pipeline audit — MPWR conversation tracking + branding fix (2026-07-01)
Full review of the portal Messaging page + guest-lifecycle SMS pipeline (`messagingEngine.js`, `bookingConfirmations.js`, `mpwrSync.js`, `scheduler.js`). Live-data finding: `messaging_config` for csr_rea has **every automation toggle OFF** (confirmations, cancellations, reminders, post-experience) as of this audit — confirmed intentional by the owner (not flipped in this session), but 3 underlying code gaps were fixed so the pipeline is correct whenever it's turned on:
1. **MPWR/Polaris (RZR) bookings never seeded a conversation.** `sendPolarisConfirmationIfNeeded` (mpwrSync.js) sent the confirmation text but skipped the equivalent of `preSeedConversation` — only the FareHarbor webhook path did this. Result: RZR guests (today's dominant booking source) never got `session_type='confirmed_guest'`, so the cancel/reschedule auto-replies never fired and the portal Conversations tab showed nothing for them. Fixed by extracting a shared `preSeedConfirmedGuestConversation(supabase, { guestPhone, toNumber, confirmationText, bookingData })` in messagingEngine.js — `bookingConfirmations.js`'s FH-specific `preSeedConversation` now delegates to it (byte-identical behavior), and `sendPolarisConfirmationIfNeeded` calls it too after a successful Polaris confirmation send. New shared `friendlyDateFromYmd` helper in mpwrSync.js (used by both the confirmation text and the pre-seed).
2. **Reminders/post-experience/30-min follow-up were invisible in the portal.** Only the initial confirmation ever got written to `conversations.messages` — scheduler.js's `processSingleMessage` just calls Twilio directly for every scheduled type. New `trackAutomatedMessage(supabase, { phone, toNumber, body, messageType })` (messagingEngine.js) does a best-effort read-then-append (not the full lock_version CAS `saveConversation` uses in index.js — deliberately lighter since these are low-frequency scheduled sends and scheduler.js runs standalone in `cron-worker.js`, which does not import index.js). Wired into scheduler.js for `reminder_24h`/`reminder_same_day`/`post_experience`/`booking_followup`, and into bookingConfirmations.js's cancellation-text send.
3. **Branding bug** — `DEFAULT_TEMPLATES` (confirmation/booking_followup/reminder_24h/reminder_same_day/post_experience) hardcoded "Colorado Sled Rentals"; a Rabbit Ears Adventures guest reminded via the FH path (or an REA past booking sent a manual post-experience text) would get the wrong brand name. Templates now use `{business}`; `resolveTemplate` defaults it to "Colorado Sled Rentals" when omitted (backward compatible). New `resolveBusinessName(shortname)` maps FH company shortnames → brand name; `scheduleReminders` derives it from `booking.company?.shortname`, `handlePortalRunPostExperience` (adminPortal.js) now selects DB2 `bookings.company` and passes it through. MPWR/RZR bookings are always genuinely CSR-branded (no REA equivalent), so `sendPolarisConfirmationIfNeeded` needed no change here.
- **Known limitation carried forward (not fixed — flagged for a future session):** `preSeedConfirmedGuestConversation` (like the prior FH-only version) REPLACES the conversation row on upsert — a repeat guest's prior thread history is not preserved across a new booking confirmation.
- All 3 fixes are inert while `messaging_config` stays all-OFF (every new code path is downstream of the existing enable_* gates or a manual admin action) — safe to deploy without changing live guest behavior. +16 tests in `test66` (messagingEngine.js) covering branding + the two new conversation-tracking helpers. 2856/2856 passing.

### Booking source + "made today" reporting (2026-06-30)
DB2 now captures, per MPWR booking, **how it came in** and **when it was actually created** — distinct from when the trip takes place. Fields on `bookings` (exposed in the `daily_manifest` view): `booked_at` (true creation timestamp from MPWR `order.createdDate`), `booking_method` (raw MPWR `order.bookingMethod`), `booking_source` (normalized bucket). Mapping (`normalizeBookingSource` in mpwrSync.js): `outfitter → walkin_phone` (staff booked it, walk-in/over-the-phone), `advWhiteLabel → online` (outfitter's own Polaris-powered site), `adv → marketplace` (Polaris Adventures consumer marketplace); unknowns pass through lowercased. Populated by `upsertOrder` every sync (active 90-day window); migration `db2_booking_source.sql` + view recreation applied to DB2.
- **New "booked" query dimension** (actionEngine `fetchBookingRows` `dateField`, intent `date_dimension:"booked"` via `isBookedDimension`). "how many bookings **made** today", "**new** bookings this week", "**created** yesterday", "**booked** today" filter by `booked_at` (not `start_at`) and add a **How they booked** source breakdown. Bare "bookings today" still = the day's manifest (trips taking place). A zero "made today" is answered as "No bookings made today yet" (no 90-day broaden). **List mode shows both dates** per line: trip date + `booked <date> (<Source>)`.
- **Briefing** `📥 BOOKED (made)` section (`getBookingsMadeStats` + `formatBookedRecently`): today/yesterday/last-7-days counts + revenue + 7d source mix + a "text 'booked today' to learn more" nudge. Suppressed for location-scoped recipients (company-wide aggregate). Injected in `buildActionCardBriefing` before the reply menu.
- **Portal** Operations → "📥 Booked (new orders)" card (`renderOpsMade`): today/yesterday/7d stat tiles + a How-they-booked bar breakdown. `GET /operations/summary` returns a pre-labeled `made` block.
- **Limitation:** the MPWR sync pulls a rolling **90-day-forward** window (by trip date), so a booking created recently for a trip >90 days out isn't captured until it enters the window; historical bookings (outside the window) have null `booked_at`/`booking_source`. Covers the near-term ("made today/yesterday/7d") case fully.
...
### Voice — listen window + operator-call exclusion (2026-06-27)
PR #23, voice.js. From a real test call that ended at ~29s. Suite 2810/2810.
- **Early-cutoff fix.** `buildGatherTwiml` set no explicit `timeout`, so Twilio's 5s default cut off a caller who paused to think after the receptionist asked a question; with the no-input limit at 2 it gave up after one reprompt and tried to transfer (to the owner's own number → fail). Added `timeout="8"` to every Gather + raised the no-input limit to 3 (two reprompts).
- **Operator exclusion.** `runMissedCallRecovery` now skips recovery SMS + conversation tracking when the caller is a registered operator/owner phone (`isRegisteredOperator`, mirrors `leads.saveLead`) — staff calling in are internal, not a guest communication. (Confirmed live: the answered-call → conversation-thread path works end-to-end; the only artifact was the owner calling from their operator number appending turns to their operator thread.)
...
- **Settings read-only view (`loadSettings`).** The General/Booking/Features `<table>` blocks now render via the shared compact `.lp-kv` list (the `row()`/`tog()` helpers emit `.lp-kv-row`; features show On/Off pills) — consistent with the lead panel. Editable form path unchanged.

### Answered-call tracking + Traffic Sources bars (2026-06-27)
PR #20. Suite 2808/2808.
- **Answered AI calls now tracked in Conversations (voice.js).** `trackRecoveryConversation` → **`trackCallConversation`** (exported): logs EVERY call into `conversations` keyed by (caller, business number) — **answered** calls record the transcript turns (`caller`→user / `agent`→assistant) under a 📞 "Called in" header; **transferred** calls note the handoff; **missed** calls keep the 📞 header + recovery text. In `runMissedCallRecovery` it runs whenever `smsSent || hasTurns || outcome==='transferred'` (was: only `smsSent`). Idempotent via `meta.recovery_done`.
- **E2E-verified.** Drove the real `handleVoiceStatus` → `runMissedCallRecovery` → `trackCallConversation` → real `saveConversation` against the **live DB** with a synthetic +1555 caller (Twilio mocked, cleaned up): produced a 📞 header + 3 transcript turns; duplicate webhook confirmed idempotency. Note: `index.js` `main()` is gated on `process.argv[1] === __filename`, so importing `getConversation`/`saveConversation` does NOT boot the server. +5 unit tests.
- **Analytics → Traffic Sources → Top Pages (public/portal.html `loadAttribution`).** Replaced the horizontal-scroll table with a ranked bar list (`.tsrc-*`): page · chats · leads (accent) · bookings (amber), bars scaled to the busiest page. Pairs with the lead-funnel chart.

### Call tracking + UX pass 4 — Campaigns/Conversations cards, dispatch actions (2026-06-27)
PR #19. Suite 2803/2803.
- **Track inbound calls + recovery texts (voice.js).** A missed-call recovery SMS was fire-and-forget — it never landed in `conversations`, so the operator never saw the bot text the caller back. `runMissedCallRecovery` now records the inbound call (📞 "Called in" guest event + AI summary) AND the recovery text into the conversation keyed by **(caller, business number)** — the same key the inbound-SMS flow uses, so a text-back lands in that thread and the bot continues. Marks `bookingData._origin='voice'`. `getConversation`/`saveConversation` threaded into `voiceDeps`; `runMissedCallRecovery` exported; gated on `smsSent` (so nothing logs in TEST_MODE). New helper `trackRecoveryConversation`. +6 tests.
- **Conversations list → cards on mobile (public/portal.html).** Rounded cards + left rail for Needs Attention (red) / Paused (orange); desktop keeps the dense 320px master-detail sidebar.
- **Campaigns → cards on mobile.** name + status pill, type · audience, last-fired · fire-count, View/Delete (`.camp-card`, shown ≤640px). Desktop keeps the table + bulk-select (`#campBulkBar` hidden on mobile to avoid duplicate checkboxes). Delete uses `--danger` (table's `--err` is undefined).
- **Operations dispatch board (public/portal.html).** The board HID Call/Text/View on mobile — the actions a dispatcher needs. Now the row stacks (time + customer on top) with a full-width row of three 42px Text/Call/View buttons below (`.ops-dispatch-actions` spans both grid cols). Desktop unchanged.
...
**Briefing handoffs exclude operators + answered threads (2026-06-29).** `detectUnresolvedHandoffs` (operatorBriefing.js) drives the "N guest messages need a reply" priority. It counted any `handoff=true / handoff_resolved=false` conversation — but staff who text the briefing system leave a stale `handoff` flag on their own thread, so the GM + ops-manager were miscounted as guests ("2 guest messages need a reply" — both phantom). Now also excludes **registered operator phones** (`getOperatorPhoneSet`), **synthetic test phones** (`isTestPhone`), and **threads where the bot/agent sent the last message** (guest already answered). Verified live: csr_rea 2 phantom → 0 real.
...
**Env (SET 2026-09-20 on Railway):** web ← `OUTBOUND_API_KEY` (random, retrievable from the Railway Variables tab), `RESEND_WEBHOOK_SECRET`, `MAILING_ADDRESS` (2151 Downhill Drive, Steamboat Springs, CO 80487), `PUBLIC_BASE_URL`; cron ← `RESEND_API_KEY`, `RESEND_FROM_EMAIL`, `MAILING_ADDRESS`, `PUBLIC_BASE_URL` (the cron previously had NO Resend vars and it drains the queue too). Resend webhook → `/webhooks/resend` created (delivered/bounced/complained/failed). **Sender:** the site's DNS is at **Cloudflare, controlled by FareHarbor's web team** (owner has no access; the SiteGround DNS editor is NOT authoritative). Until FH adds the Resend records for `coloradosledrentals.com` (registered in Resend, status not_started), campaigns + booking emails send from `OUTBOUND_FROM_EMAIL=news@usehighmark.com` (verified domain; display name = client/campaign name) with `OUTBOUND_REPLY_TO=info@coloradosledrentals.com` (both set on web+cron). `RESEND_FROM_EMAIL` is left alone (portal invites use it). Precedence in `resolveFrom()`: client's verified domain → OUTBOUND_FROM_EMAIL → RESEND_FROM_EMAIL. Once the domain verifies set `OUTBOUND_FROM_EMAIL` to `info@coloradosledrentals.com`. Old note: then set `RESEND_FROM_EMAIL` on BOTH services to `Colorado Sled Rentals <info@coloradosledrentals.com>` (currently `Highmark <invites@usehighmark.com>`). The queue worker also HOLDS marketing rows when no mailing address is configured on that service.
...
- **Persona (2026-06-28 — answer-first):** `buildReceptionistSystemPrompt` = warm, knowledgeable local who NEVER reveals it's an AI (says it's "with the team"). Rules tightened after live calls felt evasive/laggy: **ANSWER FIRST** from the KB (no stalling/deflecting), **do NOT re-greet** (the caller already heard the spoken greeting), **do NOT parrot** the caller's words back, and **at most one** short follow-up question — it must NOT end every turn with a question (the old "always end with an engaging follow-up" rule caused an interrogation feel). End-of-speech `speechTimeout` is **1s** (was 2s) so the agent jumps in the moment the caller stops talking.
...
23. **Messaging automation is OFF for csr_rea (intentional, per owner, 2026-07-01).** `messaging_config` has confirmations/cancellations/reminders/post-experience all disabled — guests currently get zero automated SMS after booking (no confirmation, reminder, or post-trip text). The underlying pipeline is correct and ready (see the 2026-07-01 messaging pipeline audit note above); it just needs the owner to flip the portal Messaging toggles on when ready. When that happens, do a controlled test-phone verification pass first (portal Settings → Messaging → Send Test Message) before it goes fully live.
24. **`preSeedConfirmedGuestConversation` replaces the conversation row on each new booking** — a repeat guest's prior thread history isn't preserved across bookings (pre-existing behavior on the FH path, now shared by the MPWR path too via the 2026-07-01 refactor). Would need a real read-modify-append instead of upsert to fix properly.
