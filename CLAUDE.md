...
- **Migration:** `db1_voice.sql` adds `voice_agents.ai_enabled boolean default false` (applied; demo agent set true). Enable per client by setting `ai_enabled=true` on the `voice_agents` row.
- **Next:** Phase 3 lead qualification (`lead_score` + missed-call SMS recovery), Phase 4 shared spam network, Phase 5 industry agents. A future latency upgrade can swap `<Gather>` for Twilio Media Streams (speech-to-speech) behind the same routes.
