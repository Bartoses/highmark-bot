...
function sendNewsletter() {
  var dry = hmApi_('POST', '/email/send', { subject: NEWSLETTER.subject, html: hmHtml_(), segment: NEWSLETTER.segment });   // dry run (default)
  var ok = true;
  try {
    var ui = SpreadsheetApp.getUi();
    ok = ui.alert('Send "' + NEWSLETTER.subject + '"?', hmDescribe_(dry) + '\n\nThis cannot be undone.', ui.ButtonSet.YES_NO) === ui.Button.YES;
  ...
  hmSay_(r.already_created ? 'This newsletter was already sent (campaign ' + r.campaign_id + '). Nothing new was sent.'
...
function sendBookingReminderEmail(bookingPk) {
  return hmApi_('POST', '/email/transactional', {
